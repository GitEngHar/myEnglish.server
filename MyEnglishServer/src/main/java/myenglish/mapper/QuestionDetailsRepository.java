...
public interface QuestionDetailsRepository {
    ...
    List<MyEnglishQuestionDetailsEntity> selectQuestionByTitleId(int questionTitleId);
    ...
    void insert_question(MyEnglishQuestionDetailsEntity myEnglishQuestionDetailsEntity);
    ...
    void update_question(MyEnglishQuestionDetailsEntity questionDetailsEntity);

    /** 問題を削除する **/
    @Delete("DELETE FROM question_details "
            + "WHERE question_title_id=#{questionTitleId} AND question_details_id=#{questionDetailsId}")
    void delete_question(MyEnglishQuestionDetailsEntity questionDetailsEntity);
}
